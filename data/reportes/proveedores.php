<?php
    ...
    {   
        ...
        function Header(){             
            ...
            while($row = pg_fetch_row($sql)){
                ...
                $this->Text(75,49,"LISTA DE PROVEEDORES",0,'C',0);
                $this->Text(75,49,"LISTA DE PROVEEDORES",0,'C',0);                                                                
            }            
    ...
    $pdf->SetX(5);
    $sql = "select identificacion,nombres_completos,tipo,telefono1,empresa,direccion,forma_pago from proveedor order by fecha_creacion asc";
    $sql = sql($conexion,$sql);          
    $pdf->Cell(25, 5, "RUC/CI",1,0, 'C',0);
    $pdf->Cell(40, 5, "NOMBRES",1,0, 'C',0);
    $pdf->Cell(25, 5, "TIPO CLIENTE",1,0, 'C',0);        
    $pdf->Cell(30, 5, utf8_decode("TELÉFONO"),1,0, 'C',0);            
    $pdf->Cell(25, 5, "EMPRESA",1,0, 'C',0);        
    $pdf->Cell(30, 5, utf8_decode("DIRECCIÓN"),1,0, 'C',0);    
    $pdf->Cell(25, 5, "FORMA PAGO",1,1, 'C',0);        

    $pdf->SetFont('Amble-Regular','',9);   
    $pdf->SetX(5);    
    while($row = pg_fetch_row($sql)){                
        $pdf->Cell(25, 5, maxCaracter(utf8_decode($row[0]),13),0,0, 'L',0);
        $pdf->Cell(40, 5, maxCaracter(utf8_decode($row[1]),20),0,0, 'L',0);
        $pdf->Cell(25, 5, maxCaracter(utf8_decode($row[2]),15),0,0, 'L',0);        
        $pdf->Cell(30, 5, maxCaracter(utf8_decode($row[3]),15),0,0, 'C',0);
        $pdf->Cell(25, 5, maxCaracter(utf8_decode($row[4]),13),0,0, 'L',0);         
        $pdf->Cell(30, 5, maxCaracter(utf8_decode($row[5]),16),0,0, 'L',0);         
