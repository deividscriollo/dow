<?php
    ...
    $pdf->SetX(5);
    $sql = "select productos.codigo,descripcion,sum(cantidad::int) as cantidad, sum(total::float) as total from productos,detalle_factura_venta where productos.id_productos = detalle_factura_venta.id_productos group by codigo,descripcion order by cantidad desc limit 20";
    $sql = sql($conexion,$sql);          
    $pdf->Cell(40, 5, "CODIGO",1,0, 'C',0);
    $pdf->Cell(80, 5, "DESCRIPCION",1,0, 'C',0);
    $pdf->Cell(20, 5, "CANTIDAD",1,0, 'C',0);    
    $pdf->Cell(40, 5, "PRECIO PROMEDIO",1,0, 'C',0);    
    ...
    while($row = pg_fetch_row($sql)){        
        ...
        $pdf->Cell(40, 5, maxCaracter(utf8_decode($row[0]),3),0,0, 'C',0);
        $pdf->Cell(80, 5, maxCaracter(utf8_decode($row[1]),40),0,0, 'L',0);
        $pdf->Cell(20, 5, maxCaracter(utf8_decode($row[2]),50),0,0, 'C',0);
        $pdf->Cell(40, 5, number_format($promedio,2),0,0, 'C',0);
